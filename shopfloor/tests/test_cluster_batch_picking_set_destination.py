...
class ClusterBatchPickingSetDestinationCase(ClusterBatchPickingCommonCase):
    ...
    def _test_set_destination(self, response_expected, picking_batch_id, move_line_id, barcode, qty):
    ...
    def test_set_quantity_no_location(self):
        self._simulate_batch_selected(self.batch, fill_stock=True)
        self.batch.move_line_ids.qty_done = 3.0

        self._test_set_destination(
            {
                "message": {
                    "message_type": "error",
            ...
            str(self.batch.move_line_ids.qty_done),
        )
